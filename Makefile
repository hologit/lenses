...
kustomize: _base-image
	docker build -t ghcr.io/hologit/lenses/kustomize:latest $@

npm-install: _base-image
	docker build -t ghcr.io/hologit/lenses/npm-install:latest $@

npm-run: _base-image
	docker build -t ghcr.io/hologit/lenses/npm-run:latest $@
